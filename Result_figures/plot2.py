...
# Filter the data
gatk_data = df[(df['workflow'] == 'GATK') & (df['filter_status'] != 'PASS')]  # GATK: Exclude PASS variants
clc_data = df[(df['workflow'] == 'CLC') & (df['filter_status'] == 'low_frequency')]  # CLC: Only low frequency variants
...
# Customize
plt.title(
    f'Variants per Sample from GATK (nonPASS) and CLC (LF)\n'
...
plt.savefig('/mnt/scratch/nazilak/Results/Number_of_variants/Default/plot2_GATK(nonPASS)_CLC(LF).png')
