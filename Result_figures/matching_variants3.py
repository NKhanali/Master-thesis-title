...
log_file_path = "/mnt/scratch/nazilak/Results/Matching_variants/matching_variants3_log.txt"

# Load the CSV file containing the variant data
df = pd.read_csv("/mnt/scratch/nazilak/Results/variant_comparison.csv")

# Filter the data to focus on GATK (nonPASS) and CLC (lowfreq) variants
gatk = df[(df['workflow'] == 'GATK') & (df['filter_status'] != 'PASS')].copy()
clc = df[(df['workflow'] == 'CLC') & (df['filter_status'] == 'low_frequency')].copy()
...
with open(log_file_path, "w") as log_file:
    with redirect_stdout(log_file):
        ...
        plt.title("Stacked Bar Plot of Matching and Unique Variants Per Sample (GATK (nonPASS) CLC (low frequency))")
        ...
        plot_path = "/mnt/scratch/nazilak/Results/Matching_variants/stacked_matching_variants3_per_sample.png"
        ...
        csv_path = "/mnt/scratch/nazilak/Results/Matching_variants/matching_variants3_per_sample.csv"
        ...
        plt.title("Venn Diagram: GATK (nonPASS) vs CLC Variants (low frequency)")

        venn_plot_path = "/mnt/scratch/nazilak/Results/Matching_variants/venn_diagram3_gatk_clc.png"
        ...
        df_matching = pd.read_csv("/mnt/scratch/nazilak/Results/Matching_variants/matching_variants3_per_sample.csv")
        ...
        plt.title("Common Variants Between GATK (nonPASS) and CLC (low frequency) Per Sample")
        plt.xticks(samples, rotation=90)
        plt.tight_layout()

        # Save the bar plot for common variants
        common_variants_plot_path = "/mnt/scratch/nazilak/Results/Matching_variants/common_variants_per_sample3.png"
